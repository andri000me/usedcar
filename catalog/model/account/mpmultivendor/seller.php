...
class ModelAccountMpmultivendorSeller extends Model {
	...
	{
		$query = $this->db->query("SELECT a.attribute_id, d.name, attribute_type,isenablesearch, attributeclass,isrange, attribute_name, event_text, maxlength, ismandatory, error_description, keyup_event_text
		FROM ". DB_PREFIX ."attribute AS a 
		Inner Join ". DB_PREFIX ."attribute_description AS d ON a.attribute_id = d.attribute_id 
		Inner join ". DB_PREFIX ."attribute_type as t  on a.attribute_id = t.attribute_id  
		WHERE d.language_id = '" . (int)$this->config->get('config_language_id') . "' order by attributeorder asc ");
		return $query->rows;
	}
	
	public function getSourceAttribute()
	{
		$query = $this->db->query("select attribute_id,attribute_value,attribute_text from ". DB_PREFIX ."attribute_source");
		return $query->rows;
	}
	public function getSourceDistrict($country, $district)
	{
		$query = $this->db->query("select city_id, type,name,postal_code from ". DB_PREFIX ."zone_district where country_id='".$country."' and district_id='".$district."' order by  type, name desc");
		return $query->rows;
	}
	public function getSellerStoreInfoByProduct($prod) { 
		$query = $this->db->query("SELECT * FROM ". DB_PREFIX ."mpseller WHERE status = '1' AND approved = '1'    AND  mpseller_id  =(select mpseller_id from ". DB_PREFIX ."product where product_id='". (int)$prod ."')");
		return $query->row;
	}

...
class ModelAccountCustomer extends Model {
	public function addCustomer($data) {
		...
		if($data['customer_group_id'] == 1)
		{
            ...
            $mailTemplateSeller = $this->db->query("select * from " . DB_PREFIX . "aab_mail_template where code = 'SELLER-REGISTERACCOUNT'");
            if(isset($mailTemplateSeller))
            {
                $mailBody = $mailTemplateSeller->rows[0]["Body"];
                $mailBody = str_replace("{0}",$data['firstname'] .' '. $data['lastname'],$mailBody);                
                ...
                $sql = "insert into " . DB_PREFIX 
                        ...
                        .$data['email']."', MailCC='', MailBCC = '".$mailTemplateSeller->rows[0]["BCC"]."', IsProcessF = 0";

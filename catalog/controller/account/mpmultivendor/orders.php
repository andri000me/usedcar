...
class ControllerAccountMpmultivendorOrders extends Controller {
	...
	public function info() {
		...
		if (!$this->customer->isLogged()) {
			$this->session->data['redirect'] = $this->url->link('account/mpmultivendor/order/info', 'order_id=' . $order_id, true);
		...
		if ($order_info) {
			...
			$data['breadcrumbs'][] = array(
				'text' => $this->language->get('text_order'),
				'href' => $this->url->link('account/mpmultivendor/order/info', 'order_id=' . $order_id, true)
			);
	...
	{
		$json = array();
		$json['Product'] = $this->request->post['Product'];
		$this->load->model('account/mpmultivendor/seller');
		$seller_info = $this->model_account_mpmultivendor_seller->getSellerStoreInfoByProduct($json['Product']);
		$json['seller']=$seller_info;
		$this->response->addHeader('Content-Type: application/json');
		$this->response->setOutput(json_encode($json));
	}
}
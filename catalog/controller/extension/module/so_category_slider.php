...
class ControllerExtensionModuleSocategoryslider extends Controller {
	...
	public function readData($setting){
		...
		if (isset($setting['pre_text']) && !empty($setting['pre_text']))
			$data['pre_text']	= trim(html_entity_decode($setting['pre_text']));
		else
			$data['pre_text']	= '';

		if (isset($setting['post_text']) && !empty($setting['post_text']))
			$data['post_text']	= trim(html_entity_decode($setting['post_text']));
		else
			$data['post_text']	= '';
		...
		if($setting['display_feature']){
			...
			if(count($setting['product_feature']) > 0){
				...
				foreach($products_deals as $product)
				{
					...
					$sold = $this->model_extension_module_so_category_slider->getUnitsSold($product_info['product_id']);
	...
	public function getProducts($category_id_list,$setting)
	{
		$filter_data = array(
			...
			'limit'        => $setting['source_limit'],
			'start' 	   => $setting['start'],
			'display_special'	=> $setting['display_special']
		);
		...
		if ($cat['count'] > 0)
		{
			...
			foreach($products_arr as $product_info)
			{
        		...
        		$specialPriceToDate = '';
        		$product_info['date_start']	= $this->model_extension_module_so_category_slider->getProductDateStart($product_info['product_id']);
        		$product_info['date_end']	= $this->model_extension_module_so_category_slider->getProductDateEnd($product_info['product_id']);
